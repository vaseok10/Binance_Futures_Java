...
public class SyncRequestImpl implements SyncRequestClient {
    ...
    @Override
    public Order postMarketOrder(String symbol, BigDecimal quantity, OrderSide orderSide, String reduceOnly) {
        quantity = quantity.signum() < 0 ? quantity.negate() : quantity;
        return RestApiInvoker.callSync(requestImpl.postIsolatedMarketOrder(symbol, orderSide, PositionSide.BOTH, OrderType.MARKET,
                valueOf(quantity), null, WorkingType.MARK_PRICE, reduceOnly));
    }

    @Override
    public Order postStopMarketOrder(String symbol, BigDecimal quantity, BigDecimal stopPrice, OrderSide orderSide, String reduceOnly) {
        quantity = quantity.signum() < 0 ? quantity.negate() : quantity;
        return RestApiInvoker.callSync(requestImpl.postIsolatedMarketOrder(symbol, orderSide, PositionSide.BOTH, OrderType.STOP_MARKET,
                valueOf(quantity), valueOf(stopPrice), WorkingType.CONTRACT_PRICE, reduceOnly));
    }

    @Override
    public Order postLimitOrder(String symbol, BigDecimal quantity, BigDecimal price, OrderSide orderSide) {
        quantity = quantity.signum() < 0 ? quantity.negate() : quantity;
        //boolean reduceOnly = quantity.multiply(price).compareTo(BigDecimal.valueOf(5)) < 0;
        return RestApiInvoker.callSync(requestImpl.postIsolatedMarketOrder(symbol, orderSide, PositionSide.BOTH, OrderType.LIMIT, valueOf(quantity),  valueOf(price), TimeInForce.GTC, true));
    }

    @Override
    public Order placeTrailingStopOrder(String symbol, BigDecimal quantity, BigDecimal price, OrderSide orderSide, Integer callbackRate) {
        quantity = quantity.signum() < 0 ? quantity.negate() : quantity;
        //boolean reduceOnly = quantity.multiply(price).compareTo(BigDecimal.valueOf(5)) < 0;
        return RestApiInvoker.callSync(requestImpl.postTrailingStopOrder(symbol, orderSide, valueOf(quantity),  valueOf(price), callbackRate));
    }
